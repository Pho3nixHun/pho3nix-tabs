...
if(this._needsShimAdoptedStyleSheets){this._needsShimAdoptedStyleSheets=!1;this.constructor._styles.forEach(s=>{const style=document.createElement("style");style.textContent=s.cssText;this.renderRoot.appendChild(style)})}}/**
   ...
   */LitElement.finalized=!0;/**
      ...
      `]}firstUpdated(){const slot=this.slotElement("tabs");slot&&slot.addEventListener("slotchange",this.slotChanged)}refreshSelection(){const tabs=this.slotted("tabs");tabs.forEach(tab=>{tab.classList[tabs[this.selected]===tab?"add":"remove"]("selected")})}slotElement(name){const slot=this.renderRoot&&this.renderRoot.querySelector(name?`slot[name="${name}"]`:"slot:not([name])");return slot}slotted(slotName){const slot=this.slotElement(slotName);return slot?slot.assignedElements():[]}get tabLocationToFlexDirection(){switch(this.tabLocation){case"left":return"row";case"right":return"row-reverse";case"bottom":return"column-reverse";case"top":default:return"column";}}render(){this.refreshSelection();return html`
   <style>
